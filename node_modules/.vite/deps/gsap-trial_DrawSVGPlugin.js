...
var P = function(t2) {
  var n2 = "undefined" != typeof window, i2 = 0 === (n2 ? window.location.href : "").indexOf(m(102, 105, 108, 101, 58, 47, 47)) || -1 !== t2.indexOf(m(108, 111, 99, 97, 108, 104, 111, 115, 116)) || b.test(t2) || (n2 ? window.location.hostname : "").split(".").pop() === m(108, 111, 99, 97, 108), r2 = [k, v, m(99, 111, 100, 101, 112, 101, 110, 46, 105, 111), m(99, 111, 100, 101, 112, 101, 110, 46, 112, 108, 117, 109, 98, 105, 110, 103), m(99, 111, 100, 101, 112, 101, 110, 46, 100, 101, 118), m(99, 111, 100, 101, 112, 101, 110, 46, 97, 112, 112), m(99, 111, 100, 101, 112, 101, 110, 46, 119, 101, 98, 115, 105, 116, 101), m(112, 101, 110, 115, 46, 99, 108, 111, 117, 100), m(99, 115, 115, 45, 116, 114, 105, 99, 107, 115, 46, 99, 111, 109), m(99, 100, 112, 110, 46, 105, 111), m(112, 101, 110, 115, 46, 105, 111), m(103, 97, 110, 110, 111, 110, 46, 116, 118), m(99, 111, 100, 101, 99, 97, 110, 121, 111, 110, 46, 110, 101, 116), m(116, 104, 101, 109, 101, 102, 111, 114, 101, 115, 116, 46, 110, 101, 116), m(99, 101, 114, 101, 98, 114, 97, 120, 46, 99, 111, 46, 117, 107), m(116, 121, 109, 112, 97, 110, 117, 115, 46, 110, 101, 116), m(116, 119, 101, 101, 110, 109, 97, 120, 46, 99, 111, 109), m(112, 108, 110, 107, 114, 46, 99, 111), m(104, 111, 116, 106, 97, 114, 46, 99, 111, 109), m(119, 101, 98, 112, 97, 99, 107, 98, 105, 110, 46, 99, 111, 109), m(97, 114, 99, 104, 105, 118, 101, 46, 111, 114, 103), m(99, 111, 100, 101, 115, 97, 110, 100, 98, 111, 120, 46, 105, 111), m(99, 115, 98, 46, 97, 112, 112), m(115, 116, 97, 99, 107, 98, 108, 105, 116, 122, 46, 99, 111, 109), m(115, 116, 97, 99, 107, 98, 108, 105, 116, 122, 46, 105, 111), m(99, 111, 100, 105, 101, 114, 46, 105, 111), m(109, 111, 116, 105, 111, 110, 116, 114, 105, 99, 107, 115, 46, 99, 111, 109), m(115, 116, 97, 99, 107, 111, 118, 101, 114, 102, 108, 111, 119, 46, 99, 111, 109), m(115, 116, 97, 99, 107, 101, 120, 99, 104, 97, 110, 103, 101, 46, 99, 111, 109), m(115, 116, 117, 100, 105, 111, 102, 114, 101, 105, 103, 104, 116, 46, 99, 111, 109), m(119, 101, 98, 99, 111, 110, 116, 97, 105, 110, 101, 114, 46, 105, 111), m(106, 115, 102, 105, 100, 100, 108, 101, 46, 110, 101, 116)], s2 = function() {
    if (n2) if ("loading" === document.readyState || "interactive" === document.readyState) document.addEventListener("readystatechange", s2);
    else {
  ...
  for (setTimeout(s2, 50); --o2 > -1; ) if (-1 !== t2.indexOf(r2[o2])) return true;
...
var S = (e2) => {
  ...
  if (i2 && (P2 || S2) || !f[c2] || (P2 = p(e2, f[c2][0]), S2 = p(e2, f[c2][1]), "rect" !== c2 && "line" !== c2 && (P2 *= 2, S2 *= 2), "line" === c2 && (v2 = p(e2, "x1"), b2 = p(e2, "y1"), P2 = Math.abs(P2 - v2), S2 = Math.abs(S2 - b2))), "path" === c2) o2 = u2.strokeDasharray, u2.strokeDasharray = "none", n2 = e2.getTotalLength() || 0, g(m2) !== g(k2) && !s && (s = 1) && _("Warning: <path> length cannot be measured when vector-effect is non-scaling-stroke and the element isn't proportionally scaled."), n2 *= (m2 + k2) / 2, u2.strokeDasharray = o2;
  else if ("rect" === c2) n2 = 2 * P2 * m2 + 2 * S2 * k2;
  else if ("line" === c2) n2 = y(v2, b2, v2 + P2, b2 + S2, m2, k2);
  else if ("polyline" === c2 || "polygon" === c2) for (r2 = e2.getAttribute("points").match(h) || [], "polygon" === c2 && r2.push(r2[0], r2[1]), n2 = 0, a2 = 2; a2 < r2.length; a2 += 2) n2 += y(r2[a2 - 2], r2[a2 - 1], r2[a2], r2[a2 + 1], m2, k2) || 0;
  else "circle" !== c2 && "ellipse" !== c2 || (l2 = P2 / 2 * m2, d2 = S2 / 2 * k2, n2 = Math.PI * (3 * (l2 + d2) - w((3 * l2 + d2) * (l2 + 3 * d2))));
  return n2 || 0;
};
var T = (e2, i2) => {
...
var O = { version: "3.12.5", name: "drawSVG", register(t2) {
  ...
  if (t2.tween._time || !a()) {
    ...
    if (o2) {
      for (t2._live && (n2 = S(t2._target), n2 !== t2._length && (i2 = n2 / t2._length, t2._length = n2, t2._offsetPT && (t2._offsetPT.s *= i2, t2._offsetPT.c *= i2), t2._dashPT ? (t2._dashPT.s *= i2, t2._dashPT.c *= i2) : t2._dash *= i2)); o2; ) o2.r(e2, o2.d), o2 = o2._next;
